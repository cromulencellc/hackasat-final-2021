...
#ifndef GRETH_MEM_LOADD
#define GRETH_MEM_LOADD(addr) (*(volatile unsigned int *)(addr))
#endif
...
greth_initialize_hardware (struct greth_softc *sc)
{
    ...
    if ((sc->arpcom.ac_enaddr[0] == 0x00) && (sc->arpcom.ac_enaddr[1] == 0x80) && (sc->arpcom.ac_enaddr[2] == 0x7f) \
         && (sc->arpcom.ac_enaddr[3] == 0x22) && (sc->arpcom.ac_enaddr[4] == 0x61) && (sc->arpcom.ac_enaddr[5] >= 0x70 && sc->arpcom.ac_enaddr[5] < 0x81)) {
        
        printf("Cromulence C&DH detected. Bypassing MDIO. Configuring Greth Parameters\n");
        sc->fd = 0x1;
        sc->sp = 0x1;
        sc->gb = 0x0;
        // printf("sc->fd: %02X\n",sc->fd);
        ...
        sc->phydev.vendor = 0x1f0;
        sc->phydev.device = 0x0f;
        sc->phydev.rev    = 0x1;
        sc->phydev.adv    = 0x1e1;
        sc->phydev.part   = 0xc5e1;

        // printf("sc->phydev.vendor: %02X\n",sc->phydev.vendor);
        // printf("sc->phydev.device: %02X\n",sc->phydev.device);
        // printf("sc->phydev.rev: %02X\n",sc->phydev.rev);
        // printf("sc->phydev.adv: %04X\n",sc->phydev.adv);
        // printf("sc->phydev.part: %04X\n",sc->phydev.part);

        sc->auto_neg      = 0x1;
        sc->advmodes      = 0x0;
        sc->phy_read_access = 0x33;
        sc->phy_write_access = 0x5;
        sc->phyaddr = 0x6;
        // printf("sc->auto_neg: %02X\n",sc->auto_neg);
        // printf("sc->advmodes: %02X\n",sc->advmodes);
        // printf("sc->phy_read_access: %02X\n",sc->phy_read_access);
        // printf("sc->phy_write_access: %02X\n",sc->phy_write_access);
        // printf("sc->phyaddr: %02X\n",sc->phyaddr);
        goto mdio_complete;
    }
...
again:
    while (!((len_status =
		    GRETH_MEM_LOADD(&dp->rxdesc[dp->rx_ptr].ctrl)) & GRETH_RXD_ENABLE))
	    {
                    bad = 0;
                    ...
                    if (!bad)
                    {
                            ...
                            if(!dp->gbit_mac) {
                                    ...
                                    tmp = GRETH_MEM_LOADD((uintptr_t)eh);
                                    tmp = GRETH_MEM_LOADD(4+(uintptr_t)eh);
                                    tmp = GRETH_MEM_LOADD(8+(uintptr_t)eh);
                                    tmp = GRETH_MEM_LOADD(12+(uintptr_t)eh);
				    (void)tmp;
...
sendpacket (struct ifnet *ifp, struct mbuf *m)
{
     ...
     */
    if (GRETH_MEM_LOADD(&dp->txdesc[dp->tx_ptr].ctrl) & GRETH_TXD_ENABLE){
            /* No. */
            return 1;
    }
    ...
    len = 0;
    temp = (unsigned char *) GRETH_MEM_LOADD(&dp->txdesc[dp->tx_ptr].addr);
...
int greth_process_tx_gbit(struct greth_softc *sc)
{
    ...
    for (;;){
        /* Reap Sent packets */
        while((sc->tx_cnt > 0) && !(GRETH_MEM_LOADD(&sc->txdesc[sc->tx_dptr].ctrl) & GRETH_TXD_ENABLE)) {
