...
/**********************************************************************/
void ReadFromMqtt(const char* Msg, size_t MsgLen, long EchoEnabled)
{
      ...
      while(!Done) {
         ...
         if (sscanf(line,"TIME %ld-%ld-%ld:%ld:%lf\n",
            ...
            if (sscanf(line,"SC[%ld].AC.EPS[%ld].Wheel_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].AC.EPS[i].Wheel_Switch = LongVal[0];
            }

            if (sscanf(line,"SC[%ld].AC.EPS[%ld].IMU_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].AC.EPS[i].IMU_Switch = LongVal[0];
            }

            if (sscanf(line,"SC[%ld].AC.EPS[%ld].StarTracker_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].AC.EPS[i].StarTracker_Switch = LongVal[0];
            }

            if (sscanf(line,"SC[%ld].AC.EPS[%ld].MTR_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].AC.EPS[i].MTR_Switch = LongVal[0];
            }

            if (sscanf(line,"SC[%ld].AC.EPS[%ld].SunSensor_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].AC.EPS[i].SunSensor_Switch = LongVal[0];
            }

            if (sscanf(line,"SC[%ld].AC.EPS[%ld].ADCS_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].AC.EPS[i].ADCS_Switch = LongVal[0];
            }

            if (sscanf(line,"SC[%ld].AC.EPS[%ld].COM_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].AC.EPS[i].COM_Switch = LongVal[0];
            }

            if (sscanf(line,"SC[%ld].AC.EPS[%ld].PAYLOAD_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].AC.EPS[i].PAYLOAD_Switch = LongVal[0];
            }
            ...
            if (sscanf(line,"SC[%ld].EPS[%ld].Wheel_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].EPS[i].Wheel_Switch = LongVal[0];
               SC[Isc].RequestStateRefresh = 1;
            }

            if (sscanf(line,"SC[%ld].EPS[%ld].IMU_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].EPS[i].IMU_Switch = LongVal[0];
               SC[Isc].RequestStateRefresh = 1;
            }

            if (sscanf(line,"SC[%ld].EPS[%ld].StarTracker_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].EPS[i].StarTracker_Switch = LongVal[0];
               SC[Isc].RequestStateRefresh = 1;
            }

            if (sscanf(line,"SC[%ld].EPS[%ld].MTR_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].EPS[i].MTR_Switch = LongVal[0];
               SC[Isc].RequestStateRefresh = 1;
            }

            if (sscanf(line,"SC[%ld].EPS[%ld].SunSensor_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].EPS[i].SunSensor_Switch = LongVal[0];
               SC[Isc].RequestStateRefresh = 1;
            }

            if (sscanf(line,"SC[%ld].EPS[%ld].ADCS_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].EPS[i].ADCS_Switch = LongVal[0];
               SC[Isc].RequestStateRefresh = 1;
            }

            if (sscanf(line,"SC[%ld].EPS[%ld].COM_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].EPS[i].COM_Switch = LongVal[0];
               SC[Isc].RequestStateRefresh = 1;
            }

            if (sscanf(line,"SC[%ld].EPS[%ld].PAYLOAD_Switch = %ld",
               &Isc,&i,
               &LongVal[0]) == 3) {
               SC[Isc].EPS[i].PAYLOAD_Switch = LongVal[0];
               SC[Isc].RequestStateRefresh = 1;
            }
            ...
            if (SC[Isc].Exists) {
               if (SC[Isc].AC.ParmDumpEnabled) {
                  ...
                  if (sscanf(line,"SC[%ld].AC.ParmLoadEnabled = %ld",
                     &Isc,
                     &LongVal[0]) == 2) {
                     SC[Isc].AC.ParmLoadEnabled = LongVal[0];
                  }

                  if (sscanf(line,"SC[%ld].AC.ParmDumpEnabled = %ld",
                     &Isc,
                     &LongVal[0]) == 2) {
                     SC[Isc].AC.ParmDumpEnabled = LongVal[0];
                  }
                  ...
                  if (sscanf(line,"SC[%ld].AC.Neps = %ld",
                     &Isc,
                     &LongVal[0]) == 2) {
                     SC[Isc].AC.Neps = LongVal[0];
                  }

                  if (sscanf(line,"SC[%ld].AC.Nsolarr = %ld",
                     &Isc,
                     &LongVal[0]) == 2) {
                     SC[Isc].AC.Nsolarr = LongVal[0];
                  }
                  ...
                  if (sscanf(line,"SC[%ld].AC.CSS[%ld].Channel = %ld",
                     &Isc,&i,
                     &LongVal[0]) == 3) {
                     SC[Isc].AC.CSS[i].Channel = LongVal[0];
                  }
                  ...
                  if (sscanf(line,"SC[%ld].AC.CSS[%ld].Calibrated = %ld",
                     &Isc,&i,
                     &LongVal[0]) == 3) {
                     SC[Isc].AC.CSS[i].Calibrated = LongVal[0];
                  }
                  ...
                  if (sscanf(line,"SC[%ld].AC.CSS[%ld].IllumBackground = %le",
                     &Isc,&i,
                     &DbleVal[0]) == 3) {
                     SC[Isc].AC.CSS[i].IllumBackground = DbleVal[0];
                  }
                  ...
                  if (sscanf(line,"SC[%ld].AC.FsCtrl.Kunl = %le",
                     &Isc,
                     &DbleVal[0]) == 2) {
                     SC[Isc].AC.FsCtrl.Kunl = DbleVal[0];
                  }

                  if (sscanf(line,"SC[%ld].AC.FsCtrl.wc = %le",
                     &Isc,
                     &DbleVal[0]) == 2) {
                     SC[Isc].AC.FsCtrl.wc = DbleVal[0];
                  }

                  if (sscanf(line,"SC[%ld].AC.FsCtrl.amax = %le",
                     &Isc,
                     &DbleVal[0]) == 2) {
                     SC[Isc].AC.FsCtrl.amax = DbleVal[0];
                  }

                  if (sscanf(line,"SC[%ld].AC.FsCtrl.vmax = %le",
                     &Isc,
                     &DbleVal[0]) == 2) {
                     SC[Isc].AC.FsCtrl.vmax = DbleVal[0];
                  }

                  if (sscanf(line,"SC[%ld].AC.FsCtrl.FrameTgt = %ld",
                     &Isc,
                     &LongVal[0]) == 2) {
                     SC[Isc].AC.FsCtrl.FrameTgt = LongVal[0];
                  }
                  ...
                  if (sscanf(line,"SC[%ld].AC.FsCtrl.qtr = %le %le %le %le",
                     ...
                     &DbleVal[2],
                     &DbleVal[3]) == 5) {
                     SC[Isc].AC.FsCtrl.qtr[0] = DbleVal[0];
                     SC[Isc].AC.FsCtrl.qtr[1] = DbleVal[1];
                     SC[Isc].AC.FsCtrl.qtr[2] = DbleVal[2];
                     SC[Isc].AC.FsCtrl.qtr[3] = DbleVal[3];
                  }
                  ...
                  if (sscanf(line,"SC[%ld].AC.FsCtrl.qtn = %le %le %le %le",
                     ...
                     &DbleVal[2],
                     &DbleVal[3]) == 5) {
                     SC[Isc].AC.FsCtrl.qtn[0] = DbleVal[0];
                     SC[Isc].AC.FsCtrl.qtn[1] = DbleVal[1];
                     SC[Isc].AC.FsCtrl.qtn[2] = DbleVal[2];
                     SC[Isc].AC.FsCtrl.qtn[3] = DbleVal[3];
                  }
                  ...
                  if (sscanf(line,"SC[%ld].AC.FsCtrl.qbl = %le %le %le %le",
                     ...
                     &DbleVal[2],
                     &DbleVal[3]) == 5) {
                     SC[Isc].AC.FsCtrl.qbl[0] = DbleVal[0];
                     SC[Isc].AC.FsCtrl.qbl[1] = DbleVal[1];
                     SC[Isc].AC.FsCtrl.qbl[2] = DbleVal[2];
                     SC[Isc].AC.FsCtrl.qbl[3] = DbleVal[3];
                  }
                  ...
                  if (sscanf(line,"SC[%ld].AC.FsCtrl.qimu = %le %le %le %le",
                     ...
                     &DbleVal[2],
                     &DbleVal[3]) == 5) {
                     SC[Isc].AC.FsCtrl.qimu[0] = DbleVal[0];
                     SC[Isc].AC.FsCtrl.qimu[1] = DbleVal[1];
                     SC[Isc].AC.FsCtrl.qimu[2] = DbleVal[2];
                     SC[Isc].AC.FsCtrl.qimu[3] = DbleVal[3];
                  }

                  if (sscanf(line,"SC[%ld].AC.FsCtrl.SunYaw = %le",
                     &Isc,
                     &DbleVal[0]) == 2) {
                     SC[Isc].AC.FsCtrl.SunYaw = DbleVal[0];
                  }

                  if (sscanf(line,"SC[%ld].AC.FsCtrl.SunYawTgt = %le",
                     &Isc,
                     &DbleVal[0]) == 2) {
                     SC[Isc].AC.FsCtrl.SunYawTgt = DbleVal[0];
                  }

                  if (sscanf(line,"SC[%ld].AC.FsCtrl.SunYawDeadband = %le",
                     &Isc,
                     &DbleVal[0]) == 2) {
                     SC[Isc].AC.FsCtrl.SunYawDeadband = DbleVal[0];
                  }

                  if (sscanf(line,"SC[%ld].AC.FsCtrl.SunYawRateDeadband = %le",
                     &Isc,
                     &DbleVal[0]) == 2) {
                     SC[Isc].AC.FsCtrl.SunYawRateDeadband = DbleVal[0];
                  }

                  if (sscanf(line,"SC[%ld].AC.FsCtrl.SunYawError = %le",
                     &Isc,
                     &DbleVal[0]) == 2) {
                     SC[Isc].AC.FsCtrl.SunYawError = DbleVal[0];
                  }
...
/* .. Refresh SC states that depend on inputs */
      ...
      SC[0].AC.MqttCtrl.CtrlReceived = TRUE;
}
