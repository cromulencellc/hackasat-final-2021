...
void InitInterProcessComm(void)
{
      ...
      for(Iipc=0;Iipc<Nipc;Iipc++) {
         ...
         #ifdef _ENABLE_MQTT_
         else if(I->Mode == IPC_MQTT) {
            I->MqttClient = InitMqttClient(I->HostName,I->Port, "MqttClient42", (void*) &I->EchoEnabled);
            if(StartMqttClient(I->MqttClient,MQTT_USER, MQTT_PASS, MQTT_TOPIC_RECV) < 0) {
               printf("Mqtt Client Connection Failure\n");
               exit(1);
            }
         }
         #endif
