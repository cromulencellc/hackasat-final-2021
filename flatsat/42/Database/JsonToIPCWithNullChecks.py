...
def WriteProlog():
      ...
      if Pipe == "Socket":
         ...
         if Prog == "Sim":
            outfile.write("#ifdef _ENABLE_MQTT_\n")
            outfile.write("int WriteToMqtt(MQTTClient client,  char **Prefix, long Nprefix, long EchoEnabled, const char* topic)\n")
         else:
            outfile.write("#ifdef _ENABLE_MQTT_\n")
            outfile.write("int WriteToMqtt(MQTTClient client, struct AcType *AC, const char* topic)\n")
...
def ReadProlog():
      ...
      if Pipe == "Socket":
         ...
         if Prog == "Sim":
            outfile.write("void ReadFromMqtt(const char* Msg, size_t MsgLen, long EchoEnabled)\n")
         else:
            outfile.write("void ReadFromMqtt(const char* Msg, size_t MsgLen, struct AcType *AC)\n")
      #endif
      ...
      if Pipe == "Socket":
         ...
         outfile.write("      if (MsgLen <= 0) return; /* Bail out if no message */\n\n")
         outfile.write("      Done = 0;\n")
         outfile.write("      Imsg = 0;\n")
         outfile.write("      while(!Done) {\n")
         outfile.write("         /* Parse lines from Msg, newline-delimited */\n")
...
def WriteEpilog():
      ...
      if Pipe == "Socket":
         ...
         outfile.write("      memcpy(&Msg[MsgLen],line,LineLen);\n")
         outfile.write("      MsgLen += LineLen;\n")
         outfile.write("      Success = MqttPublishMessage(client, Msg, MsgLen, topic);\n")
...
def MqttSimUpdateCode():
      ...
      if Pipe == "Mqtt":
         ...
         outfile.write("      for(Isc=0;Isc<Nsc;Isc++) {\n")
         outfile.write("         S = &SC[Isc];\n")
         outfile.write("         S->AC.MqttCtrl.CtrlReceived = 1;\n")
         outfile.write("         S->AC.MqttCtrl.LastSensorDataTime = S->AC.Time;\n")
         outfile.write("      }\n")

def MqttAppUpdateCode():

      global Prog, Verb, Pipe, outfile
      
      if Pipe == "Mqtt":
         outfile.write("\n/* .. Update SC MQTT states */\n\n")
         outfile.write("      AC->FsCtrl.SensorDataRecv = 1;\n")
         outfile.write("      AC->FsCtrl.SensorDataRecvCnt++;\n")
...
def ReadCodeBlock(Indent,FmtPrefix,ArrayIdx,ArgPrefix,ArgString,VarString,IdxLen,Ni,Nj,StructIdxString,Narg,FormatString):
      ...
      if Nj > 1:
         if len(ArgPrefix) > 4 and Prog == "App":
            line += "\n   "+Indent+"if ("+ArgPrefix.rstrip(".[i]")+" != NULL) {"
            for i in range (0,Ni):
               for j in range (0,Nj):
         ...
         if len(ArgPrefix) > 4 and Prog == "App":
            print(f"ArgPrefix: {ArgPrefix}, VarString: {VarString}")
            line += "\n   "+Indent+"if ("+ArgPrefix.rstrip(".[i]")+" != NULL) {"
...
def main():
      ...
      PipeList = {"Socket","Gmsec","File","Cmd","Mqtt"}
      
      infile = open('42.json','r')
