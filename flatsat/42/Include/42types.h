...
struct SCType {
   ...
   long Neps; /* Number of Electric power system */
   long Nsolarr; /* Number of Solar arrays */
   ...
   struct EPSType *EPS;          /* [*Neps*] */
   struct AccelType *Accel;      /* [*Nacc*] */
   struct SolarArrayType *SolArr;    /* [*Nsolarr*] */ 
};
...
struct EPSType {
   ...
   double CapacityUsed;          /* [[J]] */
   double SystemPower;           /* [[W]] */
   double RegulatorEfficiency;   /* Unitless */
   double BatteryChargeEfficiency; /* Unitless */
   double VoltageNoisePercentage;  /* % Noise on Voltages */
   double CurrentNoisePercentage;  /* % Noise on Currents */
...
/* voltage first order approximation
    y = ax + b
    a = -0.000289
    b =  8.345
    x = mA used per step */
