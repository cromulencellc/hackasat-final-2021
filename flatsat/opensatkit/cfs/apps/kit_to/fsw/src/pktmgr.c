...
static struct sockaddr_in SocketAddr = {0};
      ...
      if ( (SbStatus == CFE_SUCCESS) && (PktMgr->SuppressSend == FALSE) ) {
         ...
         if(PktMgr->DownlinkOn) {
            ...
            if (!PktUtil_IsPacketFiltered(PktPtr, &(PktMgr->Tbl.Pkt[AppId].Filter))) {
               ...
               if ((PktLen + 4)  < PKTMGR_OUTPUT_BUFFER_LEN) {
                  
                  CFE_PSP_MemCpy(&(PktMgr->outputBuffer),PktMgr->syncPattern,4);
                  CFE_PSP_MemCpy(&(PktMgr->outputBuffer[4]), PktPtr, PktLen);
                  
                  SocketStatus = sendto(PktMgr->TlmSockId, (char *)PktMgr->outputBuffer, PktLen+4, 0,
                                       (struct sockaddr *) &SocketAddr, sizeof(SocketAddr) );
                  if (SocketStatus != PktLen + 4) {
                     CFE_EVS_SendEvent(PKTMGR_SOCKET_SEND_ERR_EID, CFE_EVS_ERROR, "KIT_TO Socket Send Did not Return Correct Byte Count. Requested: %d, Returned %d", PktLen+4, SocketStatus);
                  }
                  ++NumPktsOutput;
                  NumBytesOutput += PktLen;

                  // OS_printf("KIT_TO Ouput MsgId 0x%04X, PktLen %d, SocketStatus: %d\n", CFE_SB_GetMsgId(PktPtr), PktLen, SocketStatus);
               } else {
                  CFE_EVS_SendEvent(PKTMGR_SOCKET_SEND_ERR_EID,CFE_EVS_ERROR,"0x%04X Packet to big, better chop that up", AppId);
               }
...
boolean PKTMGR_EnableOutputCmd(void* ObjDataPtr, const CFE_SB_MsgPtr_t MsgPtr)
{
   ...
   CFE_PSP_MemSet(&SocketAddr, 0, sizeof(SocketAddr));
...
**
*/
boolean PKTMGR_DisableOutputCmd(void* ObjDataPtr, const CFE_SB_MsgPtr_t MsgPtr)
{

   const PKTMGR_DisableOutputCmdParam *DisableOutputCmd = (const PKTMGR_EnableOutputCmdParam *) MsgPtr;
   boolean  RetStatus = TRUE;

   PktMgr->SuppressSend = TRUE;
   // CFE_EVS_SendEvent(PKTMGR_TLM_OUTPUT_ENA_INFO_EID, CFE_EVS_INFORMATION,
   //                   "Telemetry output disabled for IP %s", DisableOutputCmd->DestIp);
   CFE_ES_WriteToSysLog("Telemetry output disabled for IP %s", DisableOutputCmd->DestIp);

   /*
   ** If enabled then close existing TLM socket
   */
   if(PktMgr->DownlinkOn == TRUE) { 
