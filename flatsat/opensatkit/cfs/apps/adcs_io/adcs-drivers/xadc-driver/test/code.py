...
def send_tlm_status(time_sec, time_msec, motor_control_mode, backup_mode, motor_switch_state, hall_switch_state, encoder_switch_state, error_flag):
    ...
    print("Time {}-{}".format(time_sec, time_msec))
    output = struct.pack("H", time_sec) + struct.pack("H", time_msec) + bytearray([0, 0, motor_control_mode, status])
    return output
...
def voltage_sin(amplitude, period, t, offset):
    return amplitude*math.sin(2*math.pi*(1/period)*t) + offset

def voltage_square(amplitude, period, t, offset):
    v_temp = voltage_sin(amplitude, period, t, offset)
    v_out = 0.0
    if (v_temp >= offset):
        v_out = offset + amplitude/2
    elif v_temp < offset:
        v_out = offset - amplitude/2
    return v_out
...
analog_out_1 = AnalogOut(board.A1)
...
with I2CPeripheral(board.SCL, board.SDA, (i2c_addr,)) as device:
    while True:
        ...
        if not r:
            ...
            analog_out.value = voltage_to_dac(voltage_sin(v_out, 0.25, t, 0.3))
            analog_out_1.value = voltage_to_dac(voltage_square(v_out_1, 0.25, t, 0.6))
            t = t + time_step
            continue
                ...
                if not r.is_read:  # Main write which is Selected read
                    ...
                    if frame_id == 128:
                        ...
                        if time_msec >= 1000:
                            ...
                            time_sec += 1
                        n = r.write(bytes(send_tlm_status(time_sec, time_msec, motor_control_mode, backup_mode, motor_switch_state, hall_switch_state, encoder_switch_state, error_flag)))
                    elif frame_id == 133:
                        n = r.write(bytes(2))
                    elif frame_id == 134:
                        n = r.write(bytes(2))
