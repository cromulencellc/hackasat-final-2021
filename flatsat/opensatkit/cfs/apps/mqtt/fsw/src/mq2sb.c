...
*/
void MQ2SB_ProcessMsg(MessageData* MsgData);
...
/******************************************************************************
** Function: MqttRecvClientSubscribe
**
...
void MQ2SB_ClientSubscribe(void)
{
   ...
   for (i = 0; i < MQTT_MQ2SB_TBL_ENTRIES; i++) {
      ...
      if (Mq2SbTbl->Entry[i].MsgType == MQTT_MSG_TYPE_PRIMITIVE) {
         ...
         if (!MQTT_CLIENT_Subscribe(Mq2SbClient,
            ...
            ++SubscribeErr;
            CFE_EVS_SendEvent(MQSSB_CLIENT_SUBSCRIBE_ERR_EID, CFE_EVS_ERROR,
   ...
   CFE_EVS_SendEvent(MQ2SB_CLIENT_SUBSCRIBE_EID, CFE_EVS_INFORMATION,
...
int MQ2SB_GetTblEntryIdxByMqttTopic(const char* topic) {
   int i = 0;
   size_t topicLen = strnlen(topic, MQTT_TOPIC_LEN);
...
void MQ2SB_ProcessMsg(MessageData* MsgData)
{
   ...
   if(MsgPtr->payloadlen) {
      ...
      while ( MsgFound == FALSE && TblEnd == FALSE && i < MQTT_MQ2SB_TBL_ENTRIES) {
         ...
         if (Mq2SbTbl->Entry[i].MsgType == MQTT_MSG_TYPE_UNDEF) {
            ...
            if (strncmp(Mq2SbTbl->Entry[i].MqttTopic, MsgData->topicName->lenstring.data, TopicLen) == 0) {
               ...
               TopicStr[TopicLen] = '\0';
               CFE_EVS_SendEvent(MQTT_MSG_CALLBACK_EID, CFE_EVS_DEBUG, "MQTT_MSG_ProcessMsg: Topic=%s, SB MsgId=0x%x, Payload=%s", 
                                 TopicStr, Mq2SbTbl->Entry[i].MsgId, &MsgData->topicName->lenstring.data[TopicLen]);
            } 
            else {
               i++;
            }
      ...
      if (MsgFound) {
         ...
         SbMsgPtr = ConvertMqToCcsds(&Mq2SbTbl->Entry[i], MsgPtr->payload, MsgPtr->payloadlen);
...
void MQ2SB_ProcessSbMsg(MessageData* MsgData)
{
   ...
   if(MsgPtr->payloadlen) {
      CFE_SB_Msg_t* SbMsgPtr = MsgPtr->payload;
      uint16 sbMsgSize = CFE_SB_GetTotalMsgLength(SbMsgPtr);
      MQTT_LOG_DEBUG("Process MQTT -> SB Message. Payload Len: %d, Sb Msg Size: %d", MsgPtr->payloadlen, sbMsgSize);
      CFE_SB_MsgId_t MsgId = CFE_SB_GetMsgId(SbMsgPtr);
      if(sbMsgSize == MsgPtr->payloadlen) {
         SbMsgPtr = NULL;
         CFE_SB_InitMsg(SbMsgPtr, MsgId, MsgPtr->payloadlen, TRUE);
         
      } else {
         MQTT_LOG_ERROR("MQTT Msg payload size does not match SB header msg size. MsgId 0x%04X", MsgId);
      }
      if (SbMsgPtr != NULL) {
         CFE_PSP_MemCpy(SbMsgPtr, MsgPtr->payload, MsgPtr->payloadlen);
         CFE_SB_TimeStampMsg(SbMsgPtr);
         CFE_SB_SendMsg(SbMsgPtr);
      } 
...
boolean MQ2SB_TblEntryValidate(MQ2SB_TblEntry* Entry) {\
   ...
   if (strnlen(Entry->MqttTopic, MQTT_TOPIC_LEN) <= 0) valid &= FALSE;
   return valid;
}
...
boolean MQ2SB_LoadTbl(MQ2SB_Tbl* NewTbl)
{
   ...
   if (InvalidCnt == 0) {
      ...
      MQ2SB_InitStats(*MqttAppRunLoopMs,MQTT_STATS_STARTUP_INIT_MS);
