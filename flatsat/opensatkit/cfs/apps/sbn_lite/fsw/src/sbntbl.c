...
static SBNTBL_Class* PktTbl = NULL;
...
static boolean WriteJsonPkt(int32 FileHandle, const SBNTBL_Pkt* Pkt, boolean FirstPktWritten);
                        ...
                        SBNTBL_LoadTblEntry LoadTblEntryFunc)
{
   ...
   CFE_PSP_MemSet(PktTbl, 0, sizeof(SBNTBL_Class));
   SBNTBL_SetTblToUnused(&(PktTbl->Tbl));
   ...
   JSON_ObjConstructor(&(PktTbl->JsonObj[SBNTBL_OBJ_PKT]),
                       SBNTBL_OBJ_NAME_PKT,
                       PktCallback,
                       (void *)&(PktTbl->Tbl.Pkt));

   JSON_RegContainerCallback(JSON, &(PktTbl->JsonObj[SBNTBL_OBJ_PKT]));
   
   JSON_ObjConstructor(&(PktTbl->JsonObj[SBNTBL_OBJ_FILTER]),
                       SBNTBL_OBJ_NAME_FILTER,
                       FilterCallback,
                       (void *)&(PktTbl->Tbl.Pkt));

   JSON_RegContainerCallback(JSON, &(PktTbl->JsonObj[SBNTBL_OBJ_FILTER]));

} /* End SBNTBL_Constructor() */


/******************************************************************************
** Function: SBNTBL_SetPacketToUnused
**
**
*/
void SBNTBL_SetPacketToUnused(SBNTBL_Pkt* PktPtr)
{
   ...
   CFE_PSP_MemSet(PktPtr, 0, sizeof(SBNTBL_Pkt));

   PktPtr->StreamId    = SBNTBL_UNUSED_MSG_ID;
...
/******************************************************************************
** Function: SBNTBL_SetTblToUnused
**
**
*/
void SBNTBL_SetTblToUnused(SBNTBL_Tbl* TblPtr)
{
   ...
   CFE_PSP_MemSet(TblPtr, 0, sizeof(SBNTBL_Tbl));

   for (AppId=0; AppId < SBNTBL_MAX_APP_ID; AppId++) {
      
      TblPtr->Pkt[AppId].StreamId    = SBNTBL_UNUSED_MSG_ID;
      TblPtr->Pkt[AppId].Filter.Type = PKTUTIL_FILTER_ALWAYS;
   
   }
   
} /* End SBNTBL_SetTblToUnused() */
...
void SBNTBL_ResetStatus(void)
{
   ...
   JSON_ObjArrayReset (PktTbl->JsonObj, SBNTBL_OBJ_CNT);
    
} /* End SBNTBL_ResetStatus() */
...
boolean SBNTBL_LoadCmd(TBLMGR_Tbl *Tbl, uint8 LoadType, const char* Filename)
{
   ...
   CFE_EVS_SendEvent(SBN_LITE_INIT_DEBUG_EID, SBN_LITE_INIT_EVS_TYPE, "SBNTBL_LoadCmd() Entry. sizeof(PktTbl->Tbl) = %d\n", sizeof(PktTbl->Tbl));
   ...
   */
   ...
   if (JSON_OpenFile(JSON, Filename)) {
      ...
      CFE_EVS_SendEvent(SBN_LITE_INIT_DEBUG_EID, SBN_LITE_INIT_EVS_TYPE, "SBNTBL_LoadCmd() - Successfully prepared file %s\n", Filename);
      ...
      */
      if (PktTbl->AttrErrCnt == 0) {
         ...
         if (PktTbl->PktLoadCnt > 0) {
            ...
            if (LoadType == TBLMGR_LOAD_TBL_REPLACE) {
               ...
               if (PktTbl->JsonObj[SBNTBL_OBJ_PKT].Modified) {
                         
                  for (AppId=0; (AppId < SBNTBL_MAX_APP_ID) && (PktTbl->LastLoadStatus == TBLMGR_STATUS_VALID); AppId++) {
                          
                     if (PktTbl->Tbl.Pkt[AppId].StreamId != SBNTBL_UNUSED_MSG_ID) {

                        if (!(PktTbl->LoadTblEntryFunc)(AppId, (SBNTBL_Pkt*)PktTbl->JsonObj[SBNTBL_OBJ_PKT].UserData))
                  ...
                  CFE_EVS_SendEvent(SBNTBL_LOAD_EMPTY_ERR_EID, CFE_EVS_ERROR, "Load packet table update rejected.");
               ...
               CFE_EVS_SendEvent(SBNTBL_LOAD_TYPE_ERR_EID,CFE_EVS_ERROR,"Load packet table rejected. Invalid table command load type %d", LoadType);            
            ...
            CFE_EVS_SendEvent(SBNTBL_LOAD_UPDATE_ERR_EID, CFE_EVS_ERROR,
         ...
         if (PktTbl->PktLoadCnt != PktTbl->FilterLoadCnt) {
            ...
            CFE_EVS_SendEvent(SBNTBL_LOAD_UNDEF_FILTERS_EID, CFE_EVS_INFORMATION, 
      ...
      CFE_EVS_SendEvent(SBNTBL_LOAD_OPEN_ERR_EID,CFE_EVS_ERROR,"Load packet table open failure for file %s. File Status = %s JSMN Status = %s",
...
boolean SBNTBL_DumpCmd(TBLMGR_Tbl *Tbl, uint8 DumpType, const char* Filename)
{
   ...
   if (FileHandle >= OS_FS_SUCCESS) {
      ...
      sprintf(DumpRecord,"\n{\n\"name\": \"Software Bus Network Lite (SBN_LITE) Packet Table\",\n");
      OS_write(FileHandle,DumpRecord,strlen(DumpRecord));

      CFE_TIME_Print(SysTimeStr, CFE_TIME_GetTime());
      
      sprintf(DumpRecord,"\"description\": \"SBN_LITE dumped at %s\",\n",SysTimeStr);
      OS_write(FileHandle,DumpRecord,strlen(DumpRecord));
      ...
      */
      ...
      CFE_EVS_SendEvent(SBNTBL_CREATE_FILE_ERR_EID, CFE_EVS_ERROR,
...
static boolean WriteJsonPkt(int32 FileHandle, const SBNTBL_Pkt* Pkt, boolean FirstPktWritten) {
   ...
   if (Pkt->StreamId != SBNTBL_UNUSED_MSG_ID) {
...
static boolean PktCallback (void* UserData, int TokenIdx)
{
   ...
   PktTbl->JsonObj[SBNTBL_OBJ_PKT].Modified = FALSE; 
   
   CFE_EVS_SendEvent(SBN_LITE_INIT_DEBUG_EID, SBN_LITE_INIT_EVS_TYPE,
                     "SBNTBL.PktCallback: PktLoadCnt %d, AttrErrCnt %d, TokenIdx %d",
                     PktTbl->PktLoadCnt, PktTbl->AttrErrCnt, TokenIdx);
      
   SBNTBL_SetPacketToUnused(&Pkt); /* Default filter to always filtered in case not loaded by FilterCallback() */
   ...
   if (AttributeCnt == 4) {
      ...
      PktTbl->CurAppId = Pkt.StreamId & SBNTBL_APP_ID_MASK;     
      PktTbl->Tbl.Pkt[PktTbl->CurAppId] = Pkt;   
      
      PktTbl->JsonObj[SBNTBL_OBJ_PKT].Modified = TRUE;
      
      CFE_EVS_SendEvent(SBN_LITE_INIT_DEBUG_EID, SBN_LITE_INIT_EVS_TYPE, 
                        "SBNTBL.PktCallback (Stream ID, BufLim, Priority, Reliability): %d, %d, %d, %d",
                        Pkt.StreamId, Pkt.Qos.Priority, Pkt.Qos.Reliability, Pkt.BufLim);
      ...
      ++PktTbl->AttrErrCnt;     
      CFE_EVS_SendEvent(SBNTBL_LOAD_PKT_ATTR_ERR_EID, CFE_EVS_ERROR, "Invalid number of packet attributes %d. Should be 4.",
                        AttributeCnt);
   
   } /* End if invalid AttributeCnt */
      
   return PktTbl->JsonObj[SBNTBL_OBJ_PKT].Modified;
...
static boolean FilterCallback (void* UserData, int TokenIdx)
{
   ...
   PktTbl->JsonObj[SBNTBL_OBJ_FILTER].Modified = FALSE;
   
   CFE_EVS_SendEvent(SBN_LITE_INIT_DEBUG_EID, SBN_LITE_INIT_EVS_TYPE,
                     "SBNTBL.FilterCallback: Current ApId 0x%04X, ObjLoadCnt %d, AttrErrCnt %d, TokenIdx %d",
   ...
   if (AttributeCnt == 4) {
      ...
      PktTbl->JsonObj[SBNTBL_OBJ_FILTER].Modified = TRUE;
               
      CFE_EVS_SendEvent(SBN_LITE_INIT_DEBUG_EID, SBN_LITE_INIT_EVS_TYPE, 
                        "SBNTBL.FilterCallback (Type, X, N, O): %d, %d, %d, %d",
   ...
   return PktTbl->JsonObj[SBNTBL_OBJ_FILTER].Modified;
