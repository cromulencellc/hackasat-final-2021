...
void MQTT_SendHousekeepingPkt(void)
{
   ...
   Mqtt.HkPkt.RecvConnected = MQTT_CLIENT_OBJ->Connected;
   Mqtt.HkPkt.RecvSubscriptionCount = MQTT_CLIENT_OBJ->SubscriptionCount;
   Mqtt.HkPkt.RecvReconnectCount = MQTT_CLIENT_OBJ->ReconnectCount;
   Mqtt.HkPkt.RecvMsgCnt = MQTT_CLIENT_OBJ->MsgRecvCnt;
   Mqtt.HkPkt.PubMsgCnt = MQTT_CLIENT_OBJ->MsgPubCnt;
   ...
   if (INITBL_Constructor(&Mqtt.IniTbl, MQTT_INI_FILENAME, &IniCfgEnum)) {
      ...
      *AppMainPerfId = INITBL_GetIntConfig(INITBL_OBJ, CFG_APP_MAIN_PERF_ID);
      ...
      MQTT_CLIENT_Constructor(MQTT_CLIENT_OBJ,
                              ...
                              INITBL_GetIntConfig(INITBL_OBJ, CFG_CLIENT_KEEPALIVE),
                              MQ2SB_ProcessMsg,
                              MQTT_CLIENT_SubscribeMq2Sb);

      MQ2SB_Constructor(MQ2SB_OBJ, MQTT_CLIENT_OBJ,  (uint16) INITBL_GetIntConfig(INITBL_OBJ, CFG_CLIENT_YIELD_INTERVAL),
                           MQ2SB_GetTblPtr, MQ2SB_LoadTbl, MQ2SB_LoadTblEntry);

      SB2MQ_Constructor(SB2MQ_OBJ, MQTT_CLIENT_OBJ, (uint16) INITBL_GetIntConfig(INITBL_OBJ, CFG_CLIENT_YIELD_INTERVAL),
      ...
      OS_printf("MQTT Child Task Init Status: %d\n", Status);
   
   } /* End if INITBL Constructed */
   if (Status == CFE_SUCCESS) {


      OS_printf("Finished MQTT Client Constructors\n");
...
static boolean ChildTaskCallback(CHILDMGR_Class* ChildMgr)
{
   if (MQ2SB_OBJ->LastLoadStatus == TBLMGR_STATUS_VALID ) {
      MQTT_CLIENT_Refresh(&Mqtt.MqttClient);
   }
   return TRUE;
