                       ...
                       uint16 PipeDepth)
{
   ...
   Sb2Mq->StatsUpdateMsec = UpdateRate;
         ...
         if(!isFiltered) {
            ...
            if (PktLen <= MQTT_MSG_SB_PKT_LEN) {
               ...
               if (PubStatus) {
                  ...
                  CFE_EVS_SendEvent(SB2MQ_PUBLISH_STATUS_EID, CFE_EVS_DEBUG,
...
boolean SB2MQ_LoadTbl(SB2MQ_Tbl* NewTbl)
{
   ...
   if (FailedSubscription == 0) {
      ...
      SB2MQ_InitStats(Sb2Mq->StatsUpdateMsec,MQTT_STATS_STARTUP_INIT_MS);
