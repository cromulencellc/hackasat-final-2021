    ...
    {       
        ...
        case IND_EOF_SENT:                     
            ...
            {        
                ...
                if(cfdbg > 0)
                OS_printf("CF:EOF Sent:Trans %ld not found in Active Queue\n",TransInfo.trans.number);   
        ...
        case IND_MACHINE_DEALLOCATED:    
            ...
            if(TransInfo.final_status == FINAL_STATUS_SUCCESSFUL)
            {
                    ...
                    (TransInfo.role ==  CLASS_2_RECEIVER) )
                {
                    ...
                    // JRL - need a better fix than this, but this keeps the cpu from crashing in
                    // the event that a class 2 get file transaction is performed. Not sure if we're
                    // send an extra PDU or what, but there is some race condition here cleaning up a class 2 transaction
                    if(QueueEntryPtr != NULL) {
                        ...
                        CFE_EVS_SendEvent(CF_OUT_TRANS_FAILED_EID, CFE_EVS_ERROR, "QueueEntryPtr no longer valid");
                    }   
...
{    
    ...
    if(cfdbg > 0){
        if(CFE_TST(PduPtr->content[0],CF_PDUHDR_PDUTYPE_BIT)){
            OS_printf("CF:Sending File Data PDU, len=%lu\n",PduPtr->length);
        }else{
            OS_printf("CF:Sending ");
            CF_PrintPDUType(PduPtr->content[12],
                            PduPtr->content[13]);
            OS_printf(" PDU,len=%lu\n",PduPtr->length);
        }/* end if */
