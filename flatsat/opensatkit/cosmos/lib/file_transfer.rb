...
module Osk
   ...
   class CFDPFileTransfer < FileTransfer
      ...
      def get(flt_filename, gnd_filename, tlm_timeout = Osk::CFDP_GET_TIMEOUT)
         ...
         if(@cpu == 2)
            got_file = true
            get_file_cnt = tlm("CF2 HK_TLM_PKT TOTAL_SENT")
            seq_cnt = tlm("CF2 HK_TLM_PKT CCSDS_SEQUENCE")
            # cmd("CF2","PLAYBACK_FILE", "CLASS" => 2, "SRC_FILENAME" => flt_filename, "DEST_FILENAME" => gnd_filename)
            cmd("CF2","PLAYBACK_FILE", "SRC_FILENAME" => flt_filename, "DEST_FILENAME" => gnd_filename)
            wait("CF2 HK_TLM_PKT TOTAL_SENT == #{get_file_cnt}+1", tlm_timeout)  # Delay until put file count increments or timeout
            if (tlm("CF2 HK_TLM_PKT CCSDS_SEQUENCE") == seq_cnt)
               prompt ("No telemetry received to verify the error. Verify connection and telemetry output filter table.");
               got_file = false  
            end
         else
            ...
            # cmd("CF","PLAYBACK_FILE", "CLASS" => 2, "SRC_FILENAME" => flt_filename, "DEST_FILENAME" => gnd_filename)
      ...
      def put(gnd_filename, flt_filename, tlm_timeout = Osk::CFDP_PUT_TIMEOUT)
         ...
         if (@cpu == 2)
            put_file = true
            put_file_cnt = tlm("CF2 HK_TLM_PKT TOTAL_RECEIVED")
            seq_cnt = tlm("CF2 HK_TLM_PKT CCSDS_SEQUENCE")
            # cmd("CFDP","SEND_FILE","SRCFILENAME" => gnd_filename,"DSTFILENAME" => flt_filename,"CPU" => @cpu)
            cmd("CFDP","SEND_FILE","CLASS" => 2, "SRCFILENAME" => gnd_filename,"DSTFILENAME" => flt_filename,"CPU" => @cpu)
            wait("CF2 HK_TLM_PKT TOTAL_RECEIVED == #{put_file_cnt}+1", tlm_timeout)  # Delay until put file count increments or timeout
            if (tlm("CF2 HK_TLM_PKT CCSDS_SEQUENCE") == seq_cnt)
               prompt ("No telemetry received to verify the error. Verify connection and telemetry output filter table.");
               put_file = false  
            end
