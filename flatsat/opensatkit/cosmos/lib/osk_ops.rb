...
module Ops
   ...
   def self.get_flt_file(flt_path_filename, gnd_path_filename, cpu=1)
      got_file = false
      if (cpu == 2)
         if (Osk::system.file_transfer_adcs.get(flt_path_filename,gnd_path_filename))
   ...
   def self.get_flt_file_prompt(gnd_path = GND_SRV_DIR, cpu=1)
      ...
      got_file = get_flt_file(flt_path_filename,gnd_path_filename,cpu)
      return got_file
   end # End get_flt_file_prompt()

   def self.put_flt_file(gnd_path_filename,flt_path_filename, cpu=1)
      put_file = false
      if (cpu == 2)
         if (Osk::system.file_transfer_adcs.put(gnd_path_filename,flt_path_filename))
            @gnd_path_filename = gnd_path_filename
            @flt_path_filename = flt_path_filename
            put_file = true
         end
   ...
   # Default gnd_path should be overridden for tables that have a separate file server directory
   def self.put_flt_file_prompt(gnd_path = GND_SRV_DIR, cpu=1)
      ...
      put_file = put_flt_file(gnd_path_filename,flt_path_filename,cpu)
   ...
   #
   def self.get_flt_bin_file(flt_path_filename, gnd_rel_path, gnd_filename, tbl_mgr_def_filename, tlm_timeout=Osk::GET_FILE_TIMEOUT, cpu=1)
      if (cpu == 2)
         gnd_path_filename = "#{Cosmos::USERPATH}/#{gnd_rel_path}/#{gnd_filename}"
         if (Osk::system.file_transfer_adcs.get(flt_path_filename,gnd_path_filename,tlm_timeout))
