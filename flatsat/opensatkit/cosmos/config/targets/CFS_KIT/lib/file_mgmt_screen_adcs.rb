...
def file_mgmt_send_cmd_adcs(screen, cmd)
   ...
   if (cmd == "GET_FILE")
   ...
   elsif (cmd == "GET_FILE_ADCS")
      # Update screen with working directories to help remind user where files are stored
      Osk::Ops::set_work_dir_widget(screen, Osk::GND_SRV_DIR, Osk::FLT_SRV_DIR)
      if (Osk::Ops::get_flt_file_prompt(Osk::GND_SRV_DIR, 2))
         Osk::Ops::set_work_dir_widget(screen)
      end
   ...
   elsif (cmd == "GET_BIN_FILE_ADCS")
      # Update screen with working directories to help remind user where files are stored
      Osk::Ops::set_work_dir_widget(screen, Osk::GND_SRV_DIR, Osk::FLT_SRV_DIR)
      flt_path_filename = ask_string("Enter full FSW /path/filename.","#{Osk::FLT_SRV_DIR}/")
      gnd_filename = ask_string("Enter ground filename without path. File will be located in #{Osk::GND_SRV_DIR}.",File.basename(flt_path_filename))
      if (Osk::Ops::get_flt_bin_file(flt_path_filename, Osk::REL_SRV_DIR, gnd_filename, "corresponding", 2))
         Osk::Ops::set_work_dir_widget(screen)
      end
   elsif (cmd == "PUT_FILE")
      # Update screen with working directories to help remind user where files are stored
   ...
   elsif (cmd == "PUT_FILE_ADCS")
      # Update screen with working directories to help remind user where files are stored
      Osk::Ops::set_work_dir_widget(screen, Osk::GND_SRV_DIR, Osk::FLT_SRV_DIR)
      if (Osk::Ops::put_flt_file_prompt(Osk::GND_SRV_DIR, 2))
         Osk::Ops::set_work_dir_widget(screen)
      end
   elsif (cmd == "CREATE_DIR")
      dir_name = ask_string("Enter full path of new directory.")
      Osk::flight.send_cmd("FM2","CREATE_DIR with DIRECTORY #{dir_name}")
   elsif (cmd == "DELETE_DIR")
      dir_name = ask_string("Enter full path of directory to be deleted.")
      Osk::flight.send_cmd("FM2","DELETE_DIR with DIRECTORY #{dir_name}")  
      ...
      Osk::flight.send_cmd("FM2","SEND_DIR_PKT with DIRECTORY #{dir_name}, DIR_LIST_OFFSET #{offset}")  
   elsif (cmd == "WRITE_DIR_TO_FILE")
      flt_dir_name = ask_string("Enter full path of flight directory to be listed.",Osk::FLT_SRV_DIR)
      Osk::Ops::send_flt_bin_file_cmd("FM2", "WRITE_DIR_TO_FILE with DIRECTORY #{flt_dir_name}, ", Osk::TBL_MGR_DEF_FM2_DIR)
   elsif (cmd == "COPY_FILE")
      src_file_name = ask_string("Enter full source /path/filename.")
      dst_file_name = ask_string("Enter full destination /path/filename.")
      Osk::flight.send_cmd("FM2","COPY_FILE with SOURCE #{src_file_name}, TARGET #{dst_file_name}")  
   elsif (cmd == "MOVE_FILE")
      src_file_name = ask_string("Enter full source /path/filename.")
      dst_file_name = ask_string("Enter full destination /path/filename.")
      ...
      Osk::flight.send_cmd("FM2","MOVE_FILE with OVERWRITE #{overwrite}, SOURCE #{src_file_name}, TARGET #{dst_file_name}")  
   elsif (cmd == "RENAME_FILE")
      src_file_name = ask_string("Enter full source /path/filename.")
      dst_file_name = ask_string("Enter full destination /path/filename.")
      Osk::flight.send_cmd("FM2","RENAME_FILE with SOURCE #{src_file_name}, TARGET #{dst_file_name}")  
   elsif (cmd == "DELETE_FILE")
      file_name = ask_string("Enter full /path/filename of file to be deleted.", Osk::FLT_SRV_DIR)
      Osk::flight.send_cmd("FM2","DELETE_FILE with FILENAME #{file_name}")  
   elsif (cmd == "DELETE_ALL_FILES")
      dir_name = ask_string("Enter full path of directory to delete files.")
      Osk::flight.send_cmd("FM2","DELETE_ALL_FILES with DIRECTORY #{dir_name}")  
   elsif (cmd == "DECOMPRESS_FILE")
      src_file_name = ask_string("Enter full source /path/filename.")
      dst_file_name = ask_string("Enter full destination /path/filename.")
      Osk::flight.send_cmd("FM2","DECOMPRESS_FILE with SOURCE #{src_file_name}, TARGET #{dst_file_name}")  
      ...
      dst_file_name = ask_string("Enter full destination /path/filename.")
      Osk::flight.send_cmd("FM2","CONCAT_FILES with SOURCE1 #{src1_file_name}, SOURCE2 #{src2_file_name}, TARGET #{dst_file_name}")  
   elsif (cmd == "SEND_FILE_INFO")
      file_name = ask_string("Enter full source /path/filename.", Osk::FLT_SRV_DIR)
      spawn("ruby #{Osk::COSMOS_PKT_VIEWER} -p 'FM2 FILE_INFO_PKT'")
      Osk::flight.send_cmd("FM2","SEND_FILE_INFO with FILENAME #{file_name}, CRC 2")  
   elsif (cmd == "LIST_OPEN_FILES")
      Cosmos.run_process("ruby tools/PacketViewer -p \"FM2 OPEN_FILES_PKT\"")
      wait (2)
      Osk::flight.send_cmd("FM2","GET_OPEN_FILES")  
   else
