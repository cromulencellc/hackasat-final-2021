...
module CFDP
  ...
  class CFDPEngine
    ...
    # end function description
    def insert_transaction(type, id, classe, cpu)
      ...
      @transactions << CFDP::Transaction.new(type.upcase, id, classe, cpu)
    ...
    def handlePDUReceived(pdu_queue,cpu)
        ...
        if transaction.nil?
          ...
          if pduPacket.pduPayload.is_a?(CFDP::PDUMetadata)
            ...
            if packet_is_downlink(pduPacket)
              ...
              insert_transaction("DOWNLINK", transaction_id(pduPacket), packet_class(pduPacket), pduPacket.cpu)
              transaction = get_transaction(pduPacket)
    ...
    # end function description
    def uplinkRequest(classType, destID, sourceFileName, destFileName, cpu)
      ...
      beginTimer = Time.now
      insert_transaction("uplink", @transaction_id, classType%2, cpu)
