...
module CFDP
    ...
    ACTIONS = {
      ...
      "FINISH_LOG" => {LIMIT:FINISH_LOG_LIMIT,  TIMEOUT:FINISH_LOG_TIMEOUT}
		...
		def initialize(type, sequenceNumberHash, classe, cpu)
    ...
    def verifyMissingPDU
      ...
      return CFDP.generatePDU("NAK", @cpu, {sourceID:DESTINATIONID, destinationID:MYID, sequenceNumber:@ID[DESTINATIONID], direction:1, transmissionMode:0,
    ...
    def finish_downlink_transaction
      ...
      finishedPdu = CFDP.generatePDU("FINISHED", @cpu, {sourceID:DESTINATIONID, destinationID:MYID, conditionCode:conditionCode, endSystemStatus:1,
    ...
    def replyWithAck?(pduPacket)
      ...
      ackPdu = CFDP.generatePDU("ACK", pduPacket.cpu, directiveCode:pduPacket.pduPayload.class.class_variable_get(:@@fdCode),
